\chapter{Redes Neuronales, Modelos Generativos y sus Aplicaciones}\label{chap:redes-neuronales-y-modelos-generativos}  % MARK: Redes Neuronales y Modelos Generativos
...
\FM[inline]{¿Se podría poner en esta parte la razón por la que estoy incluyendo a las redes neuronales? ¿O explicar la forma en que estará estructurado el capítulo? (breve intro redes neuronales, GANs, WGANs, AE, WAE y por último la aplicación con baricentros de Wasserstein). \textbf{RESPUESTA: SÍ}}
...
En la siguiente subsección define formalmente una GAN, presenta los teoremas de convergencia de la GAN, y se describe el algoritmo de entrenamiento de una GAN. En las subsecciones posteriores a esta se presentan las variantes de las GAN, como las WGAN, WGAN-GP y WGAN-LP\FM{Aún no incluyo WGAN-LP, hay que ver si da tiempo de dedicar una sección a esto}, elementos necesarios para una de las partes de esta tesis.
...
\RED[inline]{Revisar esta parte!}
\begin{remark}\label{obs:modelo-generativo-push-forward}
    En el caso en que el modelo generativo $\Prob_G (\dd x \mid z)$ se mapea de forma determinista a través de $\Prob_G (\dd x \mid z) = \delta_{G(z)}(\dd x)$, donde $G\colon\cZ\to\cX$ es una función medible, entonces la Ecuación~\eqref{eq:generative-model} se simplifica a $\Prob_G (\dd x) = \pf{G} \Prob_Z (\dd x)$, donde $\pf{G}$ es el \emph{push-forward} de la función $G$. Intuitivamente, para generar una muestra del modelo generativo, se muestrea una variable latente $z$ de la distribución $\Prob_Z$, y se mapea a través de la función $G$ para obtener una muestra $x$ del modelo generativo.
\end{remark}

Por simplicidad, la función $G\colon \cZ \to \cX$ se estima a través de una red neuronal $G_\theta$, y en virtud al Teorema~\ref{thm:gan-optimal-generator}, se sabe que, en el óptimo, el discriminador también toma una forma determinista, de forma que basta con buscar una función $D\colon\cX\to [0, 1]$, la cual también se estima a través de una red neuronal $D_\varphi$.
...
\FM[inline]{Agregar una explicación de las funciones de pérdida para este caso, explicar qué cosas buscan minimizar/maximizar y describir el algoritmo de entrenamiento.}
...
\RED[inline]{
    Siento que se repite la palabra ``para'':\newline
    Se puede destacar que en este juego, el primer objetivo es obtener una estimación de la distancia de Wasserstein, \textbf{para} después utilizar esta estimación \textbf{para} minimizar la distancia entre la distribución real y la generada.
}
...
\FM[inline]{Agregar la explicación de que ahora, gracias a que se utiliza la distancia de wasserstein, estas medidas terminan convergiendo débilmente a la distribución de referencia (lo que lo hace más fácil de converjer) -> \textbf{MUCHO MUY IMPORTANTE PARA JUSTIFICAR LO QUE VIENE MÁS ABAJO EN EL BWB}}

\subsection{Wasserstein GAN con Gradiente Penalizado}\label{ssec:wgan-gp}  % MARK: Wasserstein GAN con Gradiente Penalizado
...
\section{Baricentros de Wasserstein Restringidos}\label{sec:app-bar-wass-Proyectados}  % MARK: - Section Aplicación: Baricentros de Wasserstein Restringidos
...
\RED[inline]{Hay que revisar estos dos párrafos! Ya les hice los cambios respectivos.}
...
\FM[inline]{Quizás podría ser bueno explicar cómo los autores de \cite{zhu2016generative} utilizan una GAN para aproximar la variedad de imágenes.}
...
\FM[inline]{Denotar por $\Manifold$ a la variedad o sería mejor hacer la ``confusión'' de denotarlo por $\cM$ como a los conjuntos de modelos? igualmente los modelos generan una variedad.}
